...
public class LotteryController {
	public void run() {
		...
		LotteryTicketCount lotteryTicketCount =
			new LotteryTicketCount(lotteryStore, InputView.inputManualLotteryCount());

		LotteryTickets lotteryTickets =
			LotteryService.issueLotteryTicket(
				InputView.inputManualLotteryNumbers(lotteryTicketCount), lotteryTicketCount);

		OutputView.buyLotteryTicket(lotteryTicketCount);
		OutputView.lotteryTicket(lotteryTickets);

		LastWeekLotteryTicket lastWeekWinningNumber =
			LotteryService.lastWeekWinningNumber(InputView.inputLastWeekLotteryNumbers());

		BonusBall bonusBall = new BonusBall(InputView.inputBonusBall());

		LotteryRank lotteryRank =
			new LotteryRank(LotteryService.lotteryRankingCount(lotteryTickets, lastWeekWinningNumber, bonusBall));
