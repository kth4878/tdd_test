...
- [X] 이름은 최대 5글자 이다.
- [X] 이름은 쉼표(,)로 구분한다.
- [X] 빈 값이나 null 을 이름으로 사용할수 없다.
- [X] 상품을 입력받는다.
- [X] 상품은 참가자 인원수와 동일하다.
- [X] 상품은 쉼표(,)로 구분한다.
- [X] 사다리 높이를 입력받는다.
- [X] 사다리 높이는 최소 2이상 입력받는다.
- [X] 입력받은 데이터에 맞는 사다리를 그린다.
- [X] 이름이나 상품은 입력받은 순으로 왼쪽부터 만들어진다.
- [X] 사다리 폭은 이름을 기준으로 생성된다.
- [X] 당첨 결과 확인할 이름을 입력받는다.
- [X] 결과를 확인한다.
- [X] all 이 입력되기 전까지 당첨 확인 이름을 반복해서 입력받는다.
- [X] all 은 전체 결과 확인이다.
- [X] 프로그램을 종료한다.
-------------------------------